...
const IS_MOCK = !process.env.VISION_AGENT_URL;
...
export async function POST(request: Request): Promise<Response> {
  ...
  // Mock mode — return fake session so app doesn't crash
  if (IS_MOCK) {
    console.log(
      `[agent-session] MOCK mode — returning fake session for ${callId}`,
    );
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  if (IS_MOCK) {
    console.log(
      `[agent-session] MOCK mode — fake DELETE for session ${sessionId}`,
    );
